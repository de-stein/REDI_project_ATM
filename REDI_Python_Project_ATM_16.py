...
balance_01 = float(1000)     # some balance belongs to some pin and to some account number in the table "accounts"
balance_02 = float(2000)      # for exemple: the owner of account "11" has pin "1111" and has 1000€ in the account (balance_01)
balance_03 = float(3000)
trying = 3
...
while trying != 0:
    ...
    if (data_input not in account):                # <-- while loop checks that account number and pin are correct and match. 
        ...
        if user_choise == 'd':
            ...
            print(unvalid)
            continue # <-- so user can continue working after an activity if he wants
        break # <-- if user entered wrong data three times